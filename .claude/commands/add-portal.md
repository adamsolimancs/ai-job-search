# /add-portal - Add a Portal Adapter to the Unified Job Search Skill

You are helping the user add a source adapter for a job board in their market to the repo's single `search-jobs` skill. The repo ships worked examples of the pattern (four Danish portals plus the country-agnostic `linkedin-search` and `freehire-search`), and the README invites users elsewhere to build equivalents. Investigate the portal, scaffold the adapter from the canonical structure, and test-run a live query before registering anything.

The generator is **country-agnostic**: it works for any portal in any market and language. The adapters it produces are typically market-specific and live in the user's fork (per repo policy, country-specific adapters are not merged upstream — the generator is the upstream feature, its output is yours).
...
- If `$ARGUMENTS` contains `--list`: use Glob with `.agents/skills/search-jobs/sources/*/SOURCE.md`, print a table of installed source adapters (name, market from the description, data source from `url-reference.md`), and stop.
...
2. **Adapter name** - kebab-case, suffixed `-search` (e.g. `seek-search`, `stepstone-search`). Must not collide with an existing folder in `.agents/skills/search-jobs/sources/`.
3. **Market and language** - which country/region the portal covers and what language its postings use. Record both in `SOURCE.md` so the unified skill can select and query it correctly.
...
4. **Check access requirements and terms.**
   ...
   - If robots.txt disallows the paths or the portal's terms prohibit automated access, tell the user plainly and let them decide whether to proceed for personal use. If they proceed, the generated `SOURCE.md` **must** carry a prominent personal-use-only warning (copy the tone of `linkedin-search`'s "⚠️ Personal use only" section: keep volume low, no commercial or bulk use, own responsibility).

Record everything you found - endpoints, parameters, field anchors, quirks - you will write it into `url-reference.md` in Step 3.

---

## Step 3: Scaffold the Source Adapter

**Canonical reference:** read `.agents/skills/search-jobs/sources/linkedin-search/` before generating - it is the zero-dependency worked example of this exact structure. Copy its architecture, not its LinkedIn-specific parsing.

Create `.agents/skills/search-jobs/sources/<name>/` with:

```
<name>/
├── SOURCE.md             # Adapter metadata, interface, examples, and access notes
...
### The source-adapter contract (every generated adapter MUST honor this)

These conventions make source adapters interchangeable inside the unified `search-jobs` skill:

- **Commands:** `search` and `detail <id|url>`.
- **Search flags:** `--query`/`-q`, `--jobage <days>` (posting age; map to the portal's parameter, note in `SOURCE.md` if unsupported), `--page <n>` (1-indexed), `--limit <n>` (client-side cap), `--format json|table|plain` (default `json`). Add `--location`/`-l` if the portal supports location as a parameter; if it only supports location inside the keyword query, document that in `SOURCE.md` the way `jobindex-search` does ("include the city in `--query`").
...
- **`SOURCE.md` frontmatter:** `name`, `version: 1.0.0`, `enabled: true`, and a concise `description` naming the portal, market, language, and scope. Do not add skill-triggering metadata; `search-jobs/SKILL.md` is the only trigger.
- **`SOURCE.md` body:** what the adapter searches, the personal-use warning if Step 2 found terms restrictions, command reference with flags, 4-6 usage examples using the user's market (real cities, realistic roles), output-format table, and a Notes section recording portal quirks found in Step 2.
...
Never register a source adapter that has not returned real results. Markup assumptions from Step 2 routinely miss quirks that only show up live.

1. Install dev types and typecheck:
   ```bash
   cd .agents/skills/search-jobs/sources/<name>/cli && bun install && bun run typecheck
   ```
...
1. Ask whether the user wants the new portal added to their `/scrape` search strategy. If yes:
   - The adapter is picked up automatically by `/scrape` through `.agents/skills/search-jobs/sources/*/SOURCE.md`; no additional skill or trigger wiring is needed.
...
2. Remind the user to add the install line for their own records if they maintain a fork README:
   ```bash
   cd .agents/skills/search-jobs/sources/<name>/cli && bun install
   ```
   (Skip if the skill is zero-dependency and they don't care about typecheck types.)
3. Note that the existing `search-jobs` skill will include the adapter automatically while `enabled: true`.
...
> **Portal adapter `<name>` generated and verified.**
>
> - Files: `.agents/skills/search-jobs/sources/<name>/` (`SOURCE.md`, `url-reference.md`, CLI with tests)
...
>
> Try it: `bun run .agents/skills/search-jobs/sources/<name>/cli/src/cli.ts search -q "<test query>" --format table`
>
> Per upstream policy, market-specific adapters like this live in your fork rather than being PR'd upstream. If the portal changes its markup later, `url-reference.md` records the parsing anchors to update.

---

## Design Principles

- The generator is country-agnostic; its adapter output is market-specific and stays in the user's fork. This matches the repo policy that upstream stays a universal template.
...
- The source-adapter contract keeps every generated adapter interchangeable with the shipped ones: same commands, same flags, same output shape, same error convention.
- Zero runtime dependencies by default, matching `linkedin-search` - an adapter should run on a fresh clone with nothing but `bun`.
